        ...
        function RecursiveSearch($root)
        {
                ...
                while ($res = $ilance->db->fetch_array($sql))
                {
                        ...
                        if ($this->matches[$res['varname']] == 0)
                        {
                                ...
                                //echo "<li><strong>".$this->matches[$res['varname']]." matches found for <font color='red'>".$res['varname']." (orphan) - (phraseid ".$res['id']." of ".$this->totalphrases.")</font></strong></li>";
                        }
                        else
                        {
                                //echo "<li><strong>".$this->matches[$res['varname']]." matches found for <font color='blue'>".$res['varname']."</font></strong></li>";
...
echo "<b>Orphan Templates:</b>\n<ul>\n";
$search = new RecursiveSearch(DIR_SERVER_ROOT);
echo "</ul>\n",
     "<br /><table border='0'>\n",
     "<tr><td><i>Total number of email templates found: <strong>".$search->totalphrases."</strong>, Total number of orphan email templates found:</i></td><td><strong>".$search->orphanphrases."</strong></td></tr>\n",
     "</table>";
