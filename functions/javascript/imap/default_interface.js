...
function gui_addArea(id) {
	...
	var temp = '<input type="text"  name="img_id" class="img_id" value="' + id + '" readonly="1" style="width:21px;text-align:center" />';
...
function fetch_hero_picture_info()
{
	if (xmldata.handler.readyState == 4 && xmldata.handler.status == 200)
	{
		if (xmldata.handler.responseText != '')
		{
			...
			{
				fetch_js_object('pic_displayorder').innerHTML = '<input type="text" name="sort" id="sort" value="' + response[0] + '" style="width:60px" />';
				fetch_js_object('html_container').value = response[1];
				gui_htmlBlur();
			}
			else if (xmldata.mode == 'insert')
			{
				fetch_js_object('pic_displayorder').innerHTML = '<input type="text" name="sort" id="sort" value="' + (parseInt(response[0]) + parseInt(10)) + '" style="width:60px" />';
...
function print_hero_picture_info(src, mode)
{
	xmldata = new AJAX_Handler(true);
	xmldata.src = urlencode(src);
	xmldata.mode = urlencode(mode);
	xmldata.onreadystatechange(fetch_hero_picture_info);
	xmldata.send(AJAXURL, 'do=heropicture&filename=' + xmldata.src + '&mode=' + xmldata.mode + '&s=' + ILSESSION + '&token=' + ILTOKEN);                        
}
function gui_loadImage(src, mode) {
	//reset zoom dropdown
	...
	myimgmap.loadImage(IMAGEBASE + 'heros/' + src);
	if (mode == 'load')
	...
	print_hero_picture_info(src, mode);
}
function gui_outputChanged() {
	...
	if (output == 'css') {
		...
		if (others) {
			if (!confirm_js('CSS maps only support rectangles. Are you sure you want to transform all your areas to rectangles?')) {
				output = outputmode;
				return false;
			}
			else {
		...
		else if (myimgmap.areas.length > 0 && !confirm_js('CSS maps only support rectangles. Are you sure you want to use this output mode?')) {
			output = outputmode;
			return false;
		}
