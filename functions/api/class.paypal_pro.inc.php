...
*
* @package      iLance\PaymentGateway\PayPalPro
* @version      4.0.0.8059
* @author       ILance
*/
class paypal_pro
{
	...
	public $form_validation = '<script type="text/javascript">
<!--
function validate_card_number()
{
	if (fetch_js_object(\'creditcard_number\'))
	{
		if (fetch_js_object(\'creditcard_number\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_creditcard_number\']);
				return(false);
			}	
		}
		else
...
{
	if (fetch_js_object(\'creditcard_month\'))
	{
		if (fetch_js_object(\'creditcard_month\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_creditcard_expiration_date\']);
				return(false);
			}
		}
		else
		...
		if (fetch_js_object(\'creditcard_year\'))
	{
			if (fetch_js_object(\'creditcard_year\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_creditcard_expiration_date\']);
				return(false);
			}
...
{
	if (fetch_js_object(\'creditcard_cvv2\'))
	{
		if (fetch_js_object(\'creditcard_cvv2\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_cvv2_number\']);
				return(false);
			}	
		}
		else
...
{
	if (fetch_js_object(\'creditcard_firstname\'))
	{
		if (fetch_js_object(\'creditcard_firstname\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_first_name_on_card\']);
				return(false);
			}	
		}
		else
...
{
	if (fetch_js_object(\'creditcard_lastname\'))
	{
		if (fetch_js_object(\'creditcard_lastname\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_last_name_on_card\']);
				return(false);
			}
		}
		else
...
{
	if (fetch_js_object(\'creditcard_billing\'))
	{
		if (fetch_js_object(\'creditcard_billing\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_billing_address\']);
				return(false);
			}
		}
		else
...
{
	if (fetch_js_object(\'creditcard_postal\'))
	{
		if (fetch_js_object(\'creditcard_postal\').value == \'\')
			{	
				alert_js(phrase[\'_please_enter_your_postal_on_card\']);
				return(false);
			}	
		}
		else
...
function validate_ccpayment_form()
{
	return validate_card_number() &&  validate_expiration() && validate_cvv2() && validate_firstname() && validate_lastname() && validate_billing() && validate_postal();
}
//-->
</script>';
	...
	*/
	...
	private function format_response()
	{
		...
		else if (strtoupper($this->Response["ACK"]) == 'SUCCESS' OR strtoupper($this->Response["ACK"]) == 'SUCCESSWITHWARNING')
		{
        ...
        function print_direct_payment_form($itemid, $total, $qty, $title, $orderid, $buyershipperid, $customencrypted)
        {
                global $ilpage, $ilconfig, $headinclude;
                $headinclude .= ' ' . $this->form_validation;
		$crypted = array('project_id' => $itemid);
		$crypted = encrypt_url($crypted);
		  		
  		$html = '<form name="deposit" action="' . $ilpage['merch'] . '" method="post" accept-charset="UTF-8" onsubmit="return validate_ccpayment_form();" style="margin: 0px;">
<input type="hidden" name="cmd" value="_instant-purchase-process" />
<input type="hidden" name="crypted" value="' . $crypted . '" />
<input type="hidden" name="project_id" value="' . $itemid . '" />
<input type="hidden" name="ccsubmit" value="1" />
<input type="hidden" name="total" value="' . $total . '" />
<input type="hidden" name="title" value="' . $title . '" />
<input type="hidden" name="seller_id" value="' . fetch_auction('user_id', $itemid) . '" />
<input type="hidden" name="buyer_id" value="' . $_SESSION['ilancedata']['user']['userid'] . '" />
<input type="hidden" name="qty" value="' . $qty . '" />
<input type="hidden" name="orderid" value="' . $orderid . '" />
<input type="hidden" name="buyershipperid" value="' . $buyershipperid . '" />
	...
	function print_recurring_payment_form($payer_email = '', $subscriptionid = 0, $amount = 0, $units = '', $length = 0, $title = '', $description = '', $currency = '', $customencrypted = '', $onsubmit = '', $ismodify = 0)
        {
                global $ilpage, $ilconfig, $headinclude;
                $headinclude .= ' ' . $this->form_validation;
                $html = '<form name="ilform" action="' . $ilpage['subscription'] . '" method="post" accept-charset="UTF-8" onsubmit="return validate_ccpayment_form();" style="margin: 0px;">
<input type="hidden" name="cmd" value="process_recurring_payment" />
...
<input type="hidden" name="customencrypted" value="' . $customencrypted . '" />';
                return $html;   
        }               
}
